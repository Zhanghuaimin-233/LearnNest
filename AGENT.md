...
4. 旧的严格生成路线已移除；质量模式仍是实验路线，效率模式是后续默认路线。
5. 笔记、播客稿和 TTS 分模块处理，TTS 只绑定已验证的播客稿和口播稿。
6. Provider 连接按能力和职责绑定；任务冻结绑定，所有远程调用在构造 Provider 前先完成持久化准入。
7. 不提交 `.env`、密钥、Cookie、媒体、模型、原始 Provider 请求或运行产物。
